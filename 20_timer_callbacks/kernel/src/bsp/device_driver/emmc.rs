...
// use crate::rpi::rpi4::bsp::global::EMMC;
...
use crate::{
    ...
    print, synchronization, time, warn,
};
use rpi4_constants::*;
use tock_registers::{
...
fn timer_wait_micro(delay: u64) {
    time::time_manager().spin_for(Duration::from_micros(delay));
}

/// Gets current system counter value
fn timer_get_tick_count() -> u64 {
    time::time_manager().get_sys_tick_count()
}
...
fn tick_difference(start_time: u64, tick_count: u64) -> u64 {
    ...
    (tick_diff * time::time_manager().resolution().as_nanos() as u64) / 1000 // 1 ns == 1000 us
}
...
//             // at least, we cannot do this without prior allocation. In a no_std environment,
// where             // a heap doesnt exist, this becomes a pain. We'd either have to use something
// like             // `heapless` (and predict the amount of space we'll use for allocation) or
//             // take the last resort.
//             //
//             // use `from_raw_parts_mut` to construct a mutable slice of bytes from a slice of
//             // arrays.
//             //
//             // # Safety
...
//         }
//         let res =
//             &EMMC_CONT.emmc_transfer_blocks(start_block_idx.0, num_blocks as u32, &mut buff,
// false);         match res {
//             SdResult::EMMC_OK => Ok(()),
//             _ => Err(*res),
//         }
...
impl EMMCController {
    ...
    pub fn emmc_sendcommand_p(&self, cmd: EMMCCommand<'static>, arg: u32) -> SdResult {
        ...
        // Set the argument and the command code, Some commands require a delay before reading the
        // response
            ...
            Some(CMDTM::CMD_RSPNS_TYPE::Value::CMD_48BIT_RESP) => {
                match cmd.cmd_code.read(CMDTM::CMD_INDEX) {
                    ...
                    0x03 => {
                        ...
                        // Store the card state.  Note that this is the state the card was in before
                        // the command was accepted, not the new state.
                        unsafe {
    ...
    pub fn emmc_send_app_command(&self) -> SdResult {
        ...
        if unsafe { EMMC_CARD.rca == 0 } {
            ...
            timer_wait_micro(100); // add a 100 us delay for cmds that automatically send APP_CMDs
                                   ...
                                   // If there is an RCA, include that in APP_CMD and check card
                                   // accepted it.
        } else {
    ...
    pub fn emmc_set_clock2(&self, freq: u32) -> SdResult {
        // A divisor of zero doesnt work. I think a divisor of 1 equates to half the base clock
        // rate. TODO: need to find confirmation of the above.
    ...
    pub fn emmc_app_send_op_cond(&self, arg: u32) -> SdResult {
        ...
        // A tip: adding debug/print statements after issuing this cmd may seem like the cmd
        // executes successfully. However, removing them (i.e. `debug prints`) later can
        // give us errors. Its probably because we waited a bit longer while printing. Note:
        // the above does NOT apply if you use the impl as-is.

        // In other words- issuing `APP_SEND_OP_COND`, will trigger an APP_CMD prior to sending out
        // APP_SEND_OP_COND. We must ensure a 100us delay between the 2 commands.
    ...
    pub fn emmc_init_card(&self) -> SdResult {
        ...
        // to unlock CMD42 structure [4.3.7] same as a single block write; data block
        // includes PWD setting mode, PWD len, PWD data.
